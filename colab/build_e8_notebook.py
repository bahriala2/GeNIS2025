...
> **Faites simplement `Exécution ▸ Tout exécuter`.** Rien n'est recalculé.

Chaque unité de travail est écrite sur Drive **dès qu'elle est finie**, pas à la fin de
la cellule. Une coupure ne coûte que l'ajustement en cours :

| étape | granularité de la reprise |
|---|---|
| A — verdict `Offset` | fait une fois, relu depuis l'état. **Le corpus brut n'est pas retéléchargé** |
| 7bis — bande d'environnement | par essai |
| C, D — la campagne | par ajustement |
| E — coût d'inférence | par détecteur |

La première exécution dépose aussi `2-flows.zip` sur votre Drive, pour qu'une session
suivante le copie au lieu de le retélécharger.

**Conseil pratique :** l'étape C (le classement temporel) porte la conclusion et tient en
~1 h. Si les coupures sont fréquentes, laissez-la finir avant de lancer l'étape D, qui
est la plus longue.

**À me renvoyer :** `e8_results.json` et le dossier `e8_probs/`. Même incomplets —
l'état partiel est déjà exploitable, et je vous dirai ce qu'il permet de conclure.
""")
...
# L'etat est charge ICI, avant tout le reste, pour que chaque cellule puisse
# sauter ce qui est deja fait. Colab se deconnecte ; le notebook doit repartir
...
_faits = len(STATE.get("runs", {}))
print(f"\netat retrouve : {_faits} runs deja faits")
for _e, _lab in (("offset", "verdict Offset"),
                 ("bande_environnement", "bande d'environnement"),
                 ("temoin_dur", "temoin du chemin des donnees"),
                 ("cout_corrige", "cout d'inference")):
    print(f"   {_lab:32s} {'DEJA FAIT, sera saute' if _e in STATE else 'a faire'}")
print("\nApres une deconnexion : Execution > Tout executer. Tout ce qui est")
print("deja fait est saute, rien n'est recalcule.")
''')
...
# Cette etape a besoin du corpus brut : 380 Mo a telecharger et decompresser.
# Elle ne doit donc tourner QU'UNE FOIS. Si le verdict est deja dans l'etat,
# on le relit et on ne touche pas au reseau.
import numpy as np, pandas as pd
SEUIL_POS = 0.95

if "offset" in STATE:
    _o = STATE["offset"]
    OFFSET_POSITIONNEL = _o["positionnel"]
    rhos, rmin = _o["rho_par_fichier"], _o["rho_min"]
    print("verdict Offset relu depuis l'etat, rien n'est recalcule :")
    print(f"   correlation de rang minimale {rmin:.4f}, seuil {_o['seuil']}")
else:
    ...
    if not pathlib.Path("flows").exists():
        if drive_zip.exists():
            ...
            # On le depose sur Drive : la prochaine session copiera au lieu
            # de retelecharger, et une deconnexion ne coutera plus 380 Mo.
            try:
                drive_zip.parent.mkdir(parents=True, exist_ok=True)
                shutil.copy("2-flows.zip", drive_zip)
                print(f"depose sur Drive : {drive_zip}")
            except Exception as e:
                print(f"depot sur Drive impossible ({e}), sans consequence")
    ...
    for c in sorted(CSV60):
        ...
        rhos[nom] = rho
        print(f"{nom:<32}{rho:>24.4f}{int(d['Offset'].nunique()):>10}")
        del d
    gc.collect()
    rmin = min(abs(r) for r in rhos.values())
    OFFSET_POSITIONNEL = bool(rmin > SEUIL_POS)
    ...
    STATE["offset"] = {"rho_par_fichier": rhos, "rho_min": rmin,
                       "seuil": SEUIL_POS, "positionnel": OFFSET_POSITIONNEL}
    save_state(STATE)
...
print(f"\nliste noire corrigee : {len(BLACKLIST)} colonnes  (ajoutees : {SUP})")
''')
...
# STATE et save_state sont definis en cellule 1 : le notebook doit pouvoir
# relire son etat avant meme de toucher aux donnees.
...
def campagne(variantes, protocoles, configs, etiquette=""):
    total = len(variantes) * len(protocoles) * len(configs)
    faits = sum(1 for pr in protocoles for v in variantes for c in configs
                if f"{c}|{v}|{pr}" in STATE["runs"])
    print(f"{etiquette}{faits}/{total} deja faits, {total - faits} a faire", flush=True)
    t_dep, n_fait = time.time(), 0
    for proto in protocoles:
        ...
        for var in variantes:
            ...
            for cfg in configs:
                ...
                n_fait += 1
                reste = total - faits - n_fait
                eta = (time.time() - t_dep) / n_fait * reste / 60
                print(f"   [{proto:12s}|{var:8s}] {cfg:16s} mF1 {r['macro_f1']:.4f}  "
                      f"ECE {r['ece']:.5f}->{r['ece_calibree']:.5f}  "
                      f"{r['seconds']:.0f}s   reste {reste}, ~{eta:.0f} min",
                      flush=True)
...
# Chaque essai est enregistre des qu'il est fini. Une deconnexion au
# huitieme essai ne coute que le huitieme, pas les sept precedents.
BANDE = STATE.get("bande_environnement", {}).get("par_config", {})
deja = sum(len(v["runs"]) for v in BANDE.values())
if deja:
    print(f"{deja} essai(s) deja faits, ils seront sautes\n")
for cfg in SONDES:
    vals = BANDE.get(cfg, {}).get("runs", [])
    while len(vals) < N_REPEAT:
        t0 = time.time()
        ...
        STATE["bande_environnement"] = {"par_config": BANDE, "n_repeat": N_REPEAT,
                                        "max": max(v["etendue"] for v in BANDE.values())}
        save_state(STATE)
        print(f"   {cfg:<6} essai {len(vals)}/{N_REPEAT} : {vals[-1]:.4f}"
              f"   ({time.time()-t0:.0f}s)", flush=True)

print(f"\n{'config':<8}" + "".join(f"{'essai '+str(i+1):>12}" for i in range(N_REPEAT))
      + f"{'etendue':>12}")
for cfg in SONDES:
    v = BANDE[cfg]
    print(f"{cfg:<8}" + "".join(f"{x:>12.4f}" for x in v["runs"])
          + f"{v['etendue']:>12.4f}")
BANDE_MAX = max(v["etendue"] for v in BANDE.values()) if BANDE else 0.0
STATE["bande_environnement"]["max"] = BANDE_MAX
save_state(STATE)
...
g = lambda ix: np.nan_to_num(sc.transform(X[ix][:, ci]), nan=0., posinf=0.,
                             neginf=0.).astype("float32")
Xtr, Xva, Xte = g(tr_), g(va_), g(te_)
COUT = STATE.get("cout_corrige", {})
if COUT:
    print(f"{len(COUT)} mesure(s) de cout deja faites, sautees\n")
print(f"{'configuration':<17}{'p50 (ms)':>10}{'p99 (ms)':>10}{'flux/s':>12}")
for cfg in BASE:
    if cfg == "majority" or cfg in COUT:
        continue
    nom = cfg.split("#")[0]
    try:
        if nom in SK:
        ...
        STATE["cout_corrige"] = COUT; save_state(STATE)
