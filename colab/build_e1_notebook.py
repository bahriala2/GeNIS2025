...
Même principe que le notebook d'entraînement sur `2-flows` : tout vit dans
`MyDrive/GeNIS`, la sortie se met à côté de `article1_final`, et le module est cherché
successivement dans le répertoire de travail, sur Drive sous forme de dossier, sur Drive sous
forme de zip, puis sur Zenodo en dernier recours.

```
MyDrive/GeNIS/
    2-flows.zip           <- pipeline principal
    4-preprocessed/       <- ou 4-preprocessed.zip ; ce que E1 lit
    article1_final/       <- sorties du pipeline principal
    e1/                   <- sorties de E1, creees par ce notebook
```

Les résultats et les figures étant écrits sur Drive, une déconnexion ne fait rien perdre.
""")
...
GENIS = pathlib.Path("/content/drive/MyDrive/GeNIS")     # meme dossier que le pipeline principal
WORK  = pathlib.Path("/content/genis_e1"); WORK.mkdir(parents=True, exist_ok=True)
os.chdir(WORK)
SAVE  = GENIS / "e1"                                     # a cote de article1_final
for sub in ("", "figures", "models"):
    (SAVE / sub).mkdir(parents=True, exist_ok=True)

# ---- module 4-preprocessed ----
# 1) deja decompresse ici ? 2) dossier sur Drive ? 3) zip sur Drive ? 4) Zenodo
def find_csvs(root):
    return sorted(glob.glob(str(pathlib.Path(root) / "**" / f"*{INTERVAL}-sec*.csv"), recursive=True))

csvs = find_csvs("preprocessed")
if not csvs:
    drive_dir = GENIS / "4-preprocessed"
    drive_zip = GENIS / "4-preprocessed.zip"
    if find_csvs(drive_dir):
        print("lecture directe depuis Drive")
        csvs = find_csvs(drive_dir)
    elif drive_zip.exists():
        print("copie de 4-preprocessed.zip depuis Drive...", flush=True)
        shutil.copy(drive_zip, "4-preprocessed.zip")
        subprocess.run(["unzip", "-o", "-q", "4-preprocessed.zip", "-d", "preprocessed"], check=True)
        csvs = find_csvs("preprocessed")
    else:
        print("telechargement depuis Zenodo...", flush=True)
        subprocess.run(["wget", "-q", "--show-progress", "-O", "4-preprocessed.zip",
                        "https://zenodo.org/records/14919237/files/4-preprocessed.zip?download=1"],
                       check=True)
        subprocess.run(["unzip", "-o", "-q", "4-preprocessed.zip", "-d", "preprocessed"], check=True)
        csvs = find_csvs("preprocessed")

assert csvs, (f"aucun CSV a {INTERVAL} s trouve. Attendu : un dossier 4-preprocessed ou un "
              f"4-preprocessed.zip sous {GENIS}")
print(f"\n{len(csvs)} fichier(s) a {INTERVAL} s :")
for f in csvs:
    print(f"   {pathlib.Path(f).name:<32} {os.path.getsize(f)/1e6:8.1f} Mo")
assert any("train" in pathlib.Path(f).name.lower() for f in csvs), "fichier train introuvable"
assert any("test" in pathlib.Path(f).name.lower() for f in csvs), "fichier test introuvable"
print(f"\nsortie : {SAVE}")
""")
...
code(r"""
STATE_PATH = SAVE / "e1_results.json"
...
def inventory():
    # meme principe que le 1.4 du pipeline principal : on ne devine jamais ce qui reste
    tasks = ["CategoryLabel", "SubCategoryLabel"]
    ...
    for fam, sel in (("rapides (CPU)", lambda m: m not in deep), ("profonds (GPU)", lambda m: m in deep)):
        pl = [p for p in plan if sel(p[0])]; dn = [p for p in done if sel(p[0])]
        print(f"   {fam:<16} {len(dn):>3}/{len(pl)}")
    for t in tasks:
        for c in conds:
            n = sum(1 for p in done if p[1] == c and p[2] == t)
            tot = len(models) * len(SEEDS)
            print(f"   {t:<18} {c:<15} {n:>3}/{tot}")
    return plan

if STATE["runs"]:
    inventory()
else:
    print("aucun run enregistre : campagne complete a lancer")
""")
...
ftr = pathlib.Path(next(f for f in csvs if "train" in pathlib.Path(f).name.lower()))
fte = pathlib.Path(next(f for f in csvs if "test" in pathlib.Path(f).name.lower()))
...
fig.savefig(Path(SAVE, "figures", "figE1_single_feature.png"), dpi=DPI, facecolor="white")
...
fig.savefig(Path(SAVE, "figures", "figE1_conditions.png"), dpi=DPI, facecolor="white")
...
fig.tight_layout(pad=.3)
fig.savefig(Path(SAVE, "figures", "figE1_perclass.png"), dpi=DPI, facecolor="white")
