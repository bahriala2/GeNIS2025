...
has finished. `experiments/e5/e5_results_e9.json` holds seventy-two runs, of which
fifty-four are new — three seeds at each of three intervals for the six detectors E9
adds — while the other eighteen are the two seeds already measured for LightGBM,
XGBoost and the DNN. Table 13, Section 6.4 and the recommendation of Section 8 are
computed from that file. Nothing now blocks the deposit.

One correction belongs in version 2 rather than in a footnote. The `audit.blacklist`
field of `article1_results.json`, which version 1.0.0 carries, is the **published
twelve-column list**. The capture-file test of Section 4.3 added a thirteenth column,
IdleTime, after that campaign had run, and the corrected list lives in
`experiments/e7/e7_results.json` and `experiments/e8/e8_results.json` as
`blacklist_corrigee`. `paper/blacklist.csv` was generated from the twelve-column field
and therefore marked IdleTime as retained, contradicting Table 3; it is now generated
from the corrected list, marks it `excluded_by: capture-file test`, and asserts
thirty-six exclusions rather than thirty-five. A reader of the archive should take
`blacklist_corrigee` as the feature set every reported number was produced under.
...
| `paper/blacklist.csv` | the corrected thirteen-column disposition, regenerated by `paper/make_blacklist_csv.py` |
