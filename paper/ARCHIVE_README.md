...
Three figures in the manuscript are written directly by the pipeline notebook rather
than by an offline script, because they need arrays the results file does not carry:
the reliability diagram, the confusion matrices, and the per-detector learning curves.
Their sources are `probs/` in `article1_final.zip` and `history` in the results file,
and the notebook cells that draw them are 30, 33 and 34. Copies as published are in
`figures/` and `figures_annexe/` inside `article1_final.zip`, and the manuscript
figures are also committed to the repository under `paper/figures/`.
